...
class CheckRuleCommandExecutor : CommandExecutor {
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>?): Boolean {
        ...
        if(result[0]=='F') {
            MessageUtils.sendMessage(sender,"终为白日规则检测失败")
        }
        if(result=="T") {
            MessageUtils.sendMessage(sender,"所有规则都正确，可以开始比赛")
        }else{
            MessageUtils.sendMessage(sender,"规则有错误（${result}），可以通过setrule指令自动修复")
        }
        return true
    }
}
...
class SetRuleCommandExecutor : CommandExecutor {
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>?): Boolean {
        ...
        world.time=6000
        MessageUtils.sendMessage(sender,"重置成功")
        return true
    }
}